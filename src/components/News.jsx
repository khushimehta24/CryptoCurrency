...
function News() {
    const { data: data2, isFetching2 } = useGetCryptoStatsQuery()
    const { data: data3, isFetching3 } = useGetCryptoStats2Query()

    const navigate = useNavigate()
    const [count, setCount] = useState(9)
    const [show, setShow] = useState(false)
    const handleCountTwenty = () => {
        setCount(20)
        setShow(true)
    }
    ...
    return (
        <>
            <Grid container sx={{ display: 'flex', alignItems: 'center', marginTop: '4%', justifyContent: 'space-between' }}>
                ...
                <Grid item>
                    {
                        show ? <Button onClick={handleCountNine} sx={{ color: '#b0785e' }}>Show Less</Button> : <Button onClick={handleCountTwenty} sx={{ color: '#b0785e' }}>Show More</Button>
                    }
                </Grid>
            </Grid>
            {
                        ...
                        <Box sx={{ minWidth: 120, backgroundColor: 'white', borderRadius: '5px', marginBottom: '10%' }}>
                            <FormControl fullWidth>
                                ...
                                >
                                    ...
                                    {
                                        data3?.data?.coins.map((coin) => <MenuItem value={coin.name}>{coin.name}</MenuItem>)
                                    }
                                </Select>
                            </FormControl>
                        </Box>
